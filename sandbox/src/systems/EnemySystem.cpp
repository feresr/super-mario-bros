...
void stepOnTurtle(Entity* enemy) {
    ...
    enemy->get<Engine::TextureComponent>()->id = TextureId::TURTLE_SHELL_1;
    enemy->get<Engine::TextureComponent>()->offSetY = 0;
    enemy->get<Engine::TextureComponent>()->h = TILE_SIZE;
    enemy->assign<CallbackComponent>([=]() {
        ...
        enemy->assign<CallbackComponent>([=]() {
            enemy->get<Engine::TextureComponent>()->id = TextureId::TURTLE_1;
            enemy->get<Engine::TextureComponent>()->offSetY = -TILE_SIZE;
            enemy->get<Engine::TextureComponent>()->h = TILE_SIZE * 2;
            enemy->get<Engine::TextureComponent>()->flipH = false;
...
void flipEnemy(Entity* enemy) {
    if (enemy->has<EnemyComponent>()) {
        ...
        enemy->get<Engine::TextureComponent>()->flipV = true;
...
void turtleShellInteractions(World* world, Entity* shell) {
    if (shell->hasAny<LeftCollisionComponent, RightCollisionComponent>()) {
        world->find<EnemyComponent, KineticComponent>([&](Entity* other) {
            ...
            auto transform = other->get<Engine::TransformComponent>();
            if (AABBCollision(shell->get<Engine::TransformComponent>(), other->get<Engine::TransformComponent>())) {
...
void EnemySystem::tick(World* world) {
    ...
    world->find<EnemyComponent, Engine::TransformComponent, CrushedComponent>([&](Entity* enemy) {
        auto enemyTransform = enemy->get<Engine::TransformComponent>();
        world->create()->assign<Engine::SoundComponent>(Engine::Sound::Id::STOMP);
        switch (enemy->get<EnemyComponent>()->type) {
            case Enemy::Type::GOOMBA: {
                enemy->clearComponents();
                enemy->assign<TileComponent>();
                enemy->assign<DestroyDelayedComponent>(50);
                enemy->assign<Engine::TextureComponent>(TextureId::GOOMBA_CRUSHED);
                enemy->assign<Engine::TransformComponent>(*enemyTransform);
            ...
            case Enemy::Type::TURTLE_SHELL: {
                if (enemy->get<KineticComponent>()->accX == 0) {
                    auto player = world->findFirst<PlayerComponent>()->get<Engine::TransformComponent>()->getCenterX();
                    auto turtle = enemy->get<Engine::TransformComponent>()->getCenterX();
    ...
    world->find<WalkComponent, LeftCollisionComponent>([&](Entity* entity) {
        ...
        if (entity->has<Engine::TextureComponent>() && entity->has<EnemyComponent>()) {
            entity->get<Engine::TextureComponent>()->flipH = true;
        }
    });

    // Flip Enemy by hitting block beneath
    world->find<BottomCollisionComponent, Engine::TransformComponent, KineticComponent, EnemyComponent>([&](Entity* entity) {
    ...
    world->find<WalkComponent, RightCollisionComponent>([&](Entity* entity) {
        ...
        walkComponent->speed = -std::abs(walkComponent->speed);
        if (entity->has<Engine::TextureComponent>() && entity->has<EnemyComponent>()) {
            entity->get<Engine::TextureComponent>()->flipH = false;
        }
    });
