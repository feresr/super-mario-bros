...
void MapSystem::tick(World* world) {
    auto player = world->findFirst<PlayerComponent, Engine::TransformComponent>();
    if (player)camera->x = std::max(camera->x, (int) player->get<Engine::TransformComponent>()->getCenterX());
    ...
    world->find<Engine::TransformComponent>([&](Entity* entity) {
        ...
        if (transform->right() < camera->left() - CAMERA_WORLD_OFFSET
            ...
            if (entity->has<Engine::TextComponent>()) return;
    ...
    while (!map.tiles.empty()) {
        ...
        entity->assign<Engine::TransformComponent>(tile->x, tile->y, tile->w, tile->h);
        if (tile->hasProperty(VISIBLE)) {
            entity->assign<Engine::TextureComponent>(tile->textureId);
        }
        ...
        if (tile->hasProperty(QUESTION)) {
            ...
            entity->get<Engine::TransformComponent>()->z = 0.25;
        ...
        if (tile->hasProperty(ENEMY)) {
            ...
            if (tile->textureId == TextureId::TURTLE_1) {
                ...
                entity->assign<AnimationComponent>(std::vector<TextureId>{
                                                   ...
                                                   10,
                                                   false);
                entity->get<Engine::TextureComponent>()->h = TILE_SIZE * 2;
                entity->get<Engine::TextureComponent>()->offSetY = -TILE_SIZE;
        ...
        if (tile->hasProperty(FLAG)) {
            ...
            if (tile->textureId == TextureId::FLAG_RIGHT) {
                ...
                pole->assign<Engine::TransformComponent>(tile->x, tile->y, tile->w, tile->h);
                pole->assign<TileComponent>();
                pole->assign<Engine::TextureComponent>(TextureId::FLAG_POLE);
            }
        }
