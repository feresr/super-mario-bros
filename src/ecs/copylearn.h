...
#define ECS_DEFINE_TYPE(name) ECS::Internal::TypeRegistry name::__ecs_type_reg
    ...
    */
        ...
        */
        ...
        World(Allocator alloc)
                : entAlloc(alloc), systemAlloc(alloc),
                  ...
                  subscribers({}, 0, std::hash<TypeIndex>(), std::equal_to<TypeIndex>(),
    ...
    namespace Internal {
        ...
        protected:
            virtual void destroy(World* world) {
                using ComponentAllocator = std::allocator_traits<World::EntityAllocator>::template rebind_alloc<
    ...
    template<typename... Types>
    void World::each(typename std::common_type<std::function<void(Entity*, ComponentHandle<Types>...)>>::type viewFunc,
                     bool bIncludePendingDestroy) {
    ...
    ComponentHandle<T> Entity::assign(Args&& ... args) {
        ...
        auto found = components.find(getTypeIndex<T>());
        if (found != components.end()) {
            // THIS COMPONENT (T) ALREADY EXISTS IN `components`
            ...
            // Allocate and create a new component of type T
            ComponentAllocator alloc(world->getPrimaryAllocator());
    ...
    namespace Internal {
        inline EntityIterator::EntityIterator(class World* world, size_t index, bool bIsEnd,
                                              bool bIncludePendingDestroy)
        ...
        inline EntityIterator& EntityIterator::operator++() {
            ...
            while (index < world->getCount() &&
                   (get() == nullptr || (get()->isPendingDestroy() && !bIncludePendingDestroy))) {
                ++index;
            }
        ...
        template<typename... Types>
        EntityComponentIterator<Types...>::EntityComponentIterator(World* world, size_t index, bool bIsEnd,
                                                                   bool bIncludePendingDestroy)
                : bIsEnd(bIsEnd), index(index), world(world), bIncludePendingDestroy(bIncludePendingDestroy) {
            if (index >= world->getCount())
                this->bIsEnd = true;
        ...
        EntityComponentIterator<Types...>& EntityComponentIterator<Types...>::operator++() {
            ++index;
            while (index < world->getCount() && (get() == nullptr || !get()->template has<Types...>() ||
        ...
        template<typename... Types>
        EntityComponentView<Types...>::EntityComponentView(const EntityComponentIterator<Types...>& first,
                                                           const EntityComponentIterator<Types...>& last)
                : firstItr(first), lastItr(last) {
